...
/* function distributeArticleAcrossColumns(selection) {
  // var textWidth = spacingUnit * 20;
  // var availableWidth;
...
function setupMenu() {
  ...
  var downloadOffset = 6;
  var topGridAlign = 20; // the background is offset slightly
  var menuBounds;
  var submenuOffset;
  ...
  d3.selectAll('.subMenu')
    // Remove any width these elements had before
    .style('width', null)
...
function adjustSketchedHeaders() {
  ...
  if (bounds && (bounds.left + bounds.width >= menuLeft)) {
...
function updateWindow() {
  ...
  // Adjust the left margin to round to the grid lines
  // (TODO: when I get a chance, I'd like to DIY
  // a newspaper-esque responsive layout)
  // distributeArticleAcrossColumns(d3.selectAll('.post-content > *'));
  var posts = jQuery('.post');
  if (posts.length > 0) {
    posts.css('margin-left', 'auto');
    var leftBoundary = posts[0].getBoundingClientRect().left;
    leftBoundary = Math.floor(leftBoundary / spacingUnit) * spacingUnit;
    posts.css('margin-left', leftBoundary);
  }
  ...
  opentype.load('/fonts/Sorts-Mill-Goudy/GoudyStM-Italic.otf', function (err, font) {
    if (!err) {
      headers.each(function (d, i) {
        ...
        var bounds = this.getBoundingClientRect();
        var lineHeight = parseInt(d3.select(this).style('line-height'));
